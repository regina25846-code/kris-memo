const { app, BrowserWindow, ipcMain, Tray, Menu, screen, nativeImage, Notification, desktopCapturer, clipboard, dialog, globalShortcut } = require('electron');
...
function createTray() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { label: '복원', click: async () => {
      const result = await dialog.showOpenDialog({ title: '복원할 백업 파일 선택', filters: [{ name: 'JSON 백업', extensions: ['json'] }], properties: ['openFile'] });
